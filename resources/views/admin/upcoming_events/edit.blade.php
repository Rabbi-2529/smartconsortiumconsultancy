<!-- resources/views/admin/upcoming_events/edit.blade.php -->
...
<div class="container mt-4">
    <h1>Edit Event</h1>
    <form action="{{ route('upcoming_events.update', $upcomingEvent->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="form-group">
            <label for="title">Title</label>
            <input type="text" name="title" id="title" class="form-control" value="{{ $upcomingEvent->title }}" required>
        </div>
        <div class="form-group">
            <label for="date">Date</label>
            <input type="text" name="date" id="date" class="network-info-input datepicker form-control" value="{{ $upcomingEvent->date->format('Y-m-d') }}" required>
        </div>
        <div class="form-group">
            <label for="image">Image</label>
            <input type="file" name="image" id="image" class="form-control">
            @if ($upcomingEvent->image)
                <img src="{{ asset('storage/' . $upcomingEvent->image) }}" alt="Event Image" class="mt-2" style="max-width: 200px;">
            @endif
        </div>
        <div class="form-group">
            <label for="location">Location</label>
            <input type="text" name="location" id="location" class="form-control" value="{{ $upcomingEvent->location }}" required>
        </div>
        <div class="form-group">
            <label for="video">Video URL</label>
            <input type="text" name="video" id="video" class="form-control" value="{{ $upcomingEvent->video }}">
        </div>
